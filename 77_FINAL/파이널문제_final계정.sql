...
SELECT TRUNC(MONTHS_BETWEEN(SYSDATE, ISSUE_DATE)/12,0)
FROM TB_BOOK
WHERE BOOK_NM = '아타트롤';

-- TRUNC(MONTHS_BETWEEN(SYSDATE, ISSUE_DATE)/12,0)
-- STOCK_QTY >= 90

SELECT BOOK_NM AS "도서명", STOCK_QTY AS "재고 수량", PRICE AS "가격(Org)", PRICE * 0.8 AS "가격(New)"
FROM TB_BOOK
WHERE TRUNC(MONTHS_BETWEEN(SYSDATE, ISSUE_DATE)/12,0) >= 32
AND STOCK_QTY >= 90
ORDER BY STOCK_QTY DESC, PRICE DESC, BOOK_NM DESC ;
