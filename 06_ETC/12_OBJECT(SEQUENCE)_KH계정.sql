...
/*
    ...
    [CYCLE|NOCYCLE]                   -- 값 순환여부 지정 (기본값 NOCYCLE) => 최대값 찍고 처음부터 다시 돌아와서 시작할 수 있게함
    [NOCACHE|CACHE 바이트크기]          -- 캐시메모리 할당 (기본값 CACHE 20)
    
    * 캐시메모리 : 임시공간
                 미리 발생될 값들을 생성해서 저장해두는 공간
                 매번 호출이 될 때마다 새로이 번호를 생성하는게 아니라
                 캐시메모리 공간에 미리 생선된 값들을 가져다 쓸 수 있음 (속도가 빨라짐)
                 접속이 해제되면 => 캐시 메모리에 미리 만들어둔 번호들은 다 날라감
                 번호가 일정하게 부여 안될 수 있으니 확인 잘 해야함
...
/*
    ...
    시퀀스명.NEXTVAL : 시퀀스값에 일정값을 증가시켜 발생된 값
                      현재 시퀀스값에서 INCREMENT BY 만큼 증가된 값
...
SELECT SEQ_EMPNO.CURRVAL FROM DUAL; -- 300 마지막으로 NEXTVAL에 성공한 NEXTVAL의 값

SELECT SEQ_EMPNO.NEXTVAL FROM DUAL; -- 305
SELECT SEQ_EMPNO.NEXTVAL FROM DUAL; -- 310

SELECT * FROM USER_SEQUENCES;
SELECT SEQ_EMPNO.NEXTVAL FROM DUAL; -- 지정한 MAXVALUE값을 초과했기 때문에 오류발생!! (실패!)
...
9,999,  여기까지 1000해  999,999,999,999,999,999,999,999
