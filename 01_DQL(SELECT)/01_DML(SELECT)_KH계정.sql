...
SELECT EMP_ID, EMP_NAME, SALARY, BONUS
FROM EMPLOYEE
WHERE BONUS IS NOT NULL; -- 얘를 많이 쓰긴함
-- WHERE NOT BONUS IS NULL;
-- NOT은 컬럼명 또는 IS 뒤에서 사용이 가능하다.
...
-- 직급코드가 J7이거나 J2 사원들 중 급여가 200만원 이상인 사원들의 모든 컬럼 조회
SELECT *
FROM EMPLOYEE
...
ORDER BY BONUS DESC, SALARY ASC;    -- ORDER BY 는 정렬기준 여러개 제시 가능(첫번째 기준의 컬럼값이 동일할 경우 두번째 기준의 컬럼을 가지고 한번 더 정렬)

-- 전 사원의 사원명, 연봉 조회(이때 연봉별 내림차순 정렬조회)
SELECT EMP_NAME, SALARY * 12 AS "연봉" -- 2
FROM EMPLOYEE -- 1
-- ORDER BY SALARY * 12 DESC;
-- ORDER BY 연봉 DESC; -- 3 별칭 사용 가능
ORDER BY 2 DESC; -- 컬럼순번 사용가능
