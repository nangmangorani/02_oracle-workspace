...
-- 11. TB_STUDENT / COACH_PROFESSOR_NO, TB_PROFESSOR / PROFESSOR_NO
...
SELECT 번호 || ',' || 유형이름 AS "번호, 유형이름"
FROM (SELECT CLASS_TYPE_NO AS "번호", CLASS_TYPE_NAME AS "유형이름"
FROM TB_CLASS_TYPE);
...
SELECT STUDENT_NO, STUDENT_NAME, TO_CHAR(EXTRACT(YEAR FROM TO_DATE(SUBSTR(STUDENT_SSN,1,6)))) ,PROFESSOR_NAME
FROM TB_STUDENT
JOIN TB_DEPARTMENT USING (DEPARTMENT_NO)
JOIN TB_PROFESSOR ON (COACH_PROFESSOR_NO = PROFESSOR_NO)
WHERE DEPARTMENT_NAME = '국어국문학과'
...
UPDATE TB_GRADE
SET POINT = 3.5
WHERE STUDENT_NO = 'A331101' AND CLASS_NO = (SELECT CLASS_NO
                                            FROM TB_CLASS
